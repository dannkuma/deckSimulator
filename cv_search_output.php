...
$sql_cv=$pdo->prepare('select * from general where id=? and cv=?');
$sql_cv->execute([$_SESSION['cv_search']['id'], $_SESSION['cv_search']['cv']]);
$sql_cv_cards=$pdo->prepare('select * from general where cv=?');
$sql_cv_cards->execute([$_SESSION['cv_search']['cv']]);
...
echo '<p class="illustration_header_text">声優 担当武将一覧</p>';
echo '<a class="close cv_close" href="general_search_output.php">×</a>';
echo '</div>';//header終了
foreach ($sql_cv as $row) {
    echo '<div class="cv_header">';
    echo '<p class="cv_header_name">', $row['cv'], '</p>';
    echo '</div>';
}
require 'general_list_header.php';
foreach ($sql_cv_cards as $row) {
