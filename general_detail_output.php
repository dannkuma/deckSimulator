...
echo '<p class="detail_header_text">選択中武将情報</p>';
echo '<a class="close detail_close" href="general_search_output.php">×</a>';
echo '</div>';//header終了
$sql=$pdo->prepare('select * from general where id=?');
$sql->execute([$_SESSION['detail']['id']]);
...
foreach ($sql as $row) {
    ...
    echo '</li>';//li終了
    echo '<div class="detail_two_sides_wrapper">';//表裏画像開始
    echo '<img class="detail_two_sides" alt="img" src="img/two_sides_ (', $row['id'], ').jpg">';//表裏画像
    echo '</div>';//表裏画像終了
    echo '<div class="creator_wrapper">';//イラスト、声優開始
    echo '<a class="detail_link illustration_wrapper" href="illustration_search_insert.php?illustration=', $row['illustration'], '&id=', $row['id'], '">';//イラスト開始
    echo '<p class="illustration_name">Illustration：', $row['illustration'], '</p>';
    echo '</a>';//イラスト終了
    echo '<a class="detail_link cv_wrapper" href="cv_search_insert.php?cv=', $row['cv'], '&id=', $row['id'], '">';//cv開始
    echo '<p class="cv_name">CV：', $row['cv'], '</p>';
    echo '</a>';//cv終了
    echo '</div>';//イラスト、声優終了
...
<? require 'login_confirm_function.php';?>
</html>